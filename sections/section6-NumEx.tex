...
In all experiments, the measured quantity is directly obtained from pointwise evaluations $u(\theta;p)$ of the equation's solution $u$, for $\theta$ in a set of sensors $\mc S$; consequently, the measurement operator $H$ is given by
\[
...
Picture~\ref{fig:3dLRconv} depicts the expected error per budget curves for the different training strategies. 
...
As already noticed with the expected error per budget curves, the difference among \texttt{ALR} and the fixed-tolerance strategies is less pronounced than for GPR.
...
Picture \todo{add plot} plots the samples gathered from the posteriors from \texttt{LHSGP} and \texttt{LHSLR} as well as the ground truth for one of the runs.
