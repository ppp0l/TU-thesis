...
The failure of LR on this problem can be attributed to the nature of the forward model; in fact, due to the rapid decay of the considered solution~\eqref{eq:diffusion-solution}, the steepness of every component varies greatly over the parameter space, the flatter and steeper regions are distinct for each component.
...
\end{align*}
where for simmetry reasons we can consider the derivative with respect to the first component of $x_p$ only, as the maximum is the same for all components of $\partial_{x_p, x_n} f(t,x;x_p,x_n)$, being obtained for component $h$ of $\partial_{x_p} f(t,x;x_p,x_n)$ in $x_p = x \pm \sqrt{2t} \cdot e_h$ and similarly for $x_n$.
Note that for any $(t,x) \in \mc S$ and $h \in \{1,2,3\}$, one of $(x \pm \sqrt{2t} e_h, \tilde x)$ is inside $\Theta$ for any $\tilde x \in [-1,1]^3$.
