...
We will consider training data $ D =\{ (p_i, \tau_i, y_i) \}_{i=1}^n $, where $ p_i \in \Theta $ are the input parameters, $ \tau_i \in \R_+ $ are the tolerances of the model's evaluation, and $ y_i \in \mc Y $ are the output values of the numerical evaluations of the forward model
\[ 
y_i = H(u_{\tau_i}(\cdot; p_i)) \ \text{ for every }\ i \in \{1,\ldots,n\},
\]
with measurement operator $H$~\eqref{eq:measurement-operator} and $u_{\tau}(\cdot; p)$ being the FE discretization up to tolerance $\tau$ of the solution $u(\cdot; p)$ of some PDE.
...
\begin{itemize}[font=\itshape, leftmargin=1.5cm, align=right, labelwidth=2.4cm]
    ...
    \] 
    holds, where $\nu_{d,i} \sim \mc N(0, \Sigma^2_d(\tau_i))$ for some $\Sigma_d(\tau)$, and $\nu_{d,i}$ is independent from $Y$.
\end{itemize}  
...
\[
    H(u_{\tau}(\cdot; p)) - H(u(\cdot; p)) \sim \mc N(0, \Sigma^2_d(\tau));
\] for a general GPR task, it is common to assume 
\begin{equation}\label{eq:iid-GPR-noise}
    \Sigma_d(\tau) = \tau I_{\text{dim} \mc Y}
\end{equation} 
and whenever $\tau$ is unknown it is fitted together with the kernel's hyperparameters. \newline
...
\begin{restatable}[GP posterior]{theorem}{GPpost} \label{thm:GP-posterior}
    ...
    \begin{equation} \label{eq:GP-predictive}
        \begin{gathered}
            m_D(p) = m(p) + k(p, P) \left(k(P,P) + \Sigma^2_d(\mc T) \right)^{-1} (y - m(P))  \\
            k_D(p, p') = k(p, p') - k(p, P) \left(k(P,P) + \Sigma^2_d(\mc T) \right)^{-1} k(P, p')^T            
        \end{gathered}
    \end{equation}
...
\begin{equation} \label{eq:GPR-noise-corr}
    \Sigma^2_d(\mc T) = \text{diag}(\Sigma^2_d(\tau_1)^2, \ldots, \Sigma^2_d(\tau_n)),
\end{equation}
...
\[
    (Y_{p_1}, \dots, Y_{p_n}) \mid \Lambda \sim \mc N \left( (m(p_1), \dots, m(p_n)), k_\Lambda(P,P) + \Sigma^2_d(\mc T) \right)
\]
with $k(P,P)$ and $\Sigma^2_d(\mc T)$ as given in Theorem~\ref{thm:GP-posterior};
...
\[
        Cone_{p,v,\underline L}(p') = \bigoplus_{j=1}^{\text{dim} \mc Y} \left[ v^{(j)} - L^{(j)} \|p-p'\|_\Theta, v^{(j)} + L^{(j)} \|p-p'\|_\Theta \right]
\]
...
\begin{rmk}
    ...
    Lipschitz interpolation coincides with the noise-free case of LR, $\tau_i = 0$ for every $i = 1, \dots, n$ and $ I_0 = \{ 0\}$, and the expression of $f$ can be retrieved as a particular case of the mean prediction of the LR predictive interval, which will be defined in Definition~\ref{dfn:LR} and given with an explicit formula in Proposition~\ref{prp:LR-PI}.
\end{rmk}
...
To simplify the notation, the dependence from the training data $D$, the noise model and the constants $\underline L$ is omitted.
\begin{rmk}
...
\begin{prp}\label{prp:LR-PI}
    Let $I_{\tau_i} = \oplus_{j=1}^{\text{dim}\mc Y} [ -\tau_i^{(j)}, \tau_i^{(j)} ]$ for every $i = 1, \dots, n$.
...
\begin{cor} \label{cor:LR-const}
    ...
    In order for the training data to be feasible, i.e. it is compatible with the LR assumptions making the predictive interval $\mc IP_p$ is non-empty for every $p\in \Theta$, it is necessary that Equation~\eqref{eq:LR-const} holds for all $j = 1, \dots, \text{dim} \mc Y$.\\
