...
This is particularly useful in the context of BIPs, as the uncertainty of the surrogate model can be incorporated in to the problem's likelihood, as illustrated in Sections. \newline
...
\begin{dfn} [Gaussian Process]
    A Gaussian process (GP) over $\mc Y$ is a collection $\{Y_p\}_{p\in\Theta}$ of $\mc Y$-valued random variables, such that for any finite subset $p_1, \ldots, p_n \in \Theta$, the random variables $Y_{p_1}, \ldots, Y_{p_n}$ have a joint Gaussian distribution.

    A Gaussian process is completely defined by its mean function $m: \Theta \rightarrow \mc Y$ and its covariance function $k: \Theta \times \Theta \rightarrow \mc Y \times \mc Y$, also known as kernel. 
...
\begin{itemize}[font=\itshape, leftmargin=1.5cm, align=right, labelwidth=2.4cm]
    ...
    \item[(GPR-data)]the observations are contaminated by Gaussian noise, i.e. for $ (p,\tau,y) $ in $D$, \[ y = y(p) + \epsilon \] holds, where $\epsilon \sim \mc N(0, \Sigma(\tau)^2)$ for some $\Sigma(\tau)$.
\end{itemize}  
The first assumption implicitly prescribes a certain regularity of the forward model depending on the choice of the kernel $k$, but does not impose any parametric form on the model. \newline
The second assumption allows to derive a closed-form expression for the predictive distribution of the GP, and implies that the discretization error on numerical evaluations of the forward model is Gaussian, i.e. \[
    H(f_{\tau}(\cdot; p)) - H(f(\cdot; p)) \sim \mc N(0, \Sigma(\tau)^2).
\] 
