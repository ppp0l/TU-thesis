...
\begin{restatable}[Expected error reduction]{prp}{EER} \label{prp:EER}
    ...
    Then, the expected error reduction in $p'$ is given by \begin{equation}\label{eq:alpha-LR}
        \alpha_{D, \text{LR}}(p, p') =  \frac{1}{4 \sqrt{3} \tau_p } \sum_{j=1}^{\text{dim}\mc Y} \frac{EUI(p,p')^{(j)} - ELI(p,p')^{(j)}}{UB^{(j)}(p) - LB^{(j)}(p)} ,
    \end{equation}
    ...
    \begin{equation*}
        ...
        ELI(p,p')^{(j)}  = 
        \begin{cases}
            2\tau_p(c_2^j (UB^{(j)}(p) -LB^{(j)}(p)) - \dfrac{1}{2} (LB^{(j)}(p)^2 - UB^{(j)}(p)^2)) &
            \text{if } \ LB^{(j)}(p) \geq - c_2^j + \tau_p  
            \\[3ex]

            \dfrac{1}{6}((c_2^j + UB^{(j)}(p) - \tau_p)^3 - (c_2^j + LB^{(j)}(p) + \tau_p)^3)&
            \begin{aligned}
                \text{if } & \ - c_2^j - \tau_p > LB^{(j)}(p) \\
                &\text{ and } UB^{(j)}(p) \geq - c_2^j + \tau_p
            \end{aligned}
            \\[4ex]

            -\dfrac{1}{6}(c_2^j+\tau_p+UB^{(j)}(p))^3 &
            \begin{aligned}
                \text{if } & \ - c_2^j - \tau_p > LB^{(j)}(p) \\
                &\text{ and } - c_2^j + \tau_p > UB^{(j)}(p) \geq - c_2^j - \tau_p
            \end{aligned}
            \\[4ex]

            \dfrac{1}{6}((c_2^j + LB^{(j)}(p) + \tau_p)^3 - (c_2^j + UB^{(j)}(p) + \tau_p)^3) &
            \begin{aligned}
                \text{if } & \ - c_2^j + \tau_p > LB^{(j)}(p) \geq - c_2^j - \tau_p \\
                &\text{ and } - c_2^j + \tau_p > UB^{(j)}(p) \geq - c_2^j - \tau_p
            \end{aligned}
            \\[4ex]

            \begin{aligned}
                2\tau_p(c_2^j (-c_2^j + \tau_p -UB^{(j)}(p)) &- \dfrac{1}{2} (UB^{(j)}(p)^2 - (c_2^j-\tau_p)^2)) +\\
                &+ \dfrac{1}{6} ( (c_2^j +LB^{(j)}(p) + \tau_p)^3 -8\tau_p^3)
            \end{aligned}&
            \begin{aligned}
                \text{if } & \ LB^{(j)}(p) \geq - c_2^j - \tau_p \\
                &\text{ and } UB^{(j)}(p) \geq - c_2^j + \tau_p
